...
final class HttpModrinthTransport implements ModrinthTransport {
    ...
    public void download(URI uri, Path destination, long maximumBytes) throws IOException, InterruptedException {
        HttpResponse<InputStream> response = send(uri, Duration.ofMinutes(5));
    ...
    private HttpResponse<InputStream> send(URI uri, Duration timeout)
        ...
        try {
            ...
            throw transientFailure("request", uri, exception);
        }
    ...
    private static RetryableRepositoryException transientFailure(String operation, URI uri,
            IOException cause) {
        return new RetryableRepositoryException("Modrinth " + operation + " failed for "
                + uri.getPath(), cause);
    }
}
