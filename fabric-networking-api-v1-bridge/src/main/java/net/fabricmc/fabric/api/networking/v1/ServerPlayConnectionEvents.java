...
public final class ServerPlayConnectionEvents {
    ...
    public static final Event<Join> JOIN = EventFactory.createArrayBacked(Join.class,
            callbacks -> (handler, sender, server) -> {
                for (Join callback : callbacks) callback.onPlayReady(handler, sender, server);
            });
    public static final Event<Disconnect> DISCONNECT = EventFactory.createArrayBacked(Disconnect.class,
            callbacks -> (handler, server) -> {
                for (Disconnect callback : callbacks) callback.onPlayDisconnect(handler, server);
            });
