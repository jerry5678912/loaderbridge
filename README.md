...
paths. It also observed each side's final Minecraft launch arguments. On the
client, Fabric initialization received the real `Minecraft` instance; on the
dedicated server, the instance was correctly null during initialization and
became the constructed server at Forge's about-to-start boundary. Both sides
